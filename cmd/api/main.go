...
func run() error {
	...
	statistics, err := stats.NewService(repository.NewMySQLStatsRepository(mysqlDB), teamRepository)
	if err != nil {
		return err
	}
	...
	server := &http.Server{
		...
		Handler:           httpapi.NewHandler(users, teams, tasks, comments, statistics, tokens),
